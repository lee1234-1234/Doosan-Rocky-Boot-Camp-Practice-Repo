...
class LibraryGUI:
    def __init__(self, root):
        ...
        self.startup_message = self.capture_print(self.load_library_data)

        self.tester = TestCase(self.library)
        self.report = ReportClass(self.library, "library_log.txt")
    ...
    def on_borrow(self):
        ...
        confirm = messagebox.askyesno("대여 확인", 
                                      f"{user_name}님 {book_title} 책을 대여하시겠습니까?")
        if not confirm:
            self.show_message("대여 처리를 취소했습니다.", target="rent")
            return
        
        message = self.capture_print(
            self.tester.borrow_selected_book_for_gui
        )
    ...
    #TODO 8
    def on_report_top_books(self):
        message = self.capture_print(
            self.report.show_top3_borrowed_books
        )
        self.show_message(message, target="report")

    def on_report_top_users(self):
        message = self.capture_print(
            self.report.show_top3_borrow_users
        )
        self.show_message(message, target="report")

    def on_report_recent_returns(self):
        message = self.capture_print(
            self.report.show_recent5_return_logs
        )
        self.show_message(message, target="report")
    ...
    def show_rent_return_page(self):
        ...
        #TODO 3
        user_id_btn = self.make_button(frame,
                                       "회원 ID 조회",
                                       command = self.on_user_search)
        ...
        #TODO 5
        book_id_btn = self.make_button(frame,
                                       "책 ID 조회",
                                       command = self.on_book_search)
